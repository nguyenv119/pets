...
function makeElements(): {
  ...
  const maxEl = { textContent: '' } as unknown as HTMLElement;
  const nextEl = { textContent: '' } as unknown as HTMLElement;
  return { countEl, maxEl, nextEl };
}
...
describe('renderCapacityCounter — full capacity (at cap)', () => {
  it('shows CAPACITY_CAP count and empty next-el when at capacity cap', async () => {
     ...
     * CAPACITY_CAP (7), the count element shows "7", max shows "7", and the
     * next-slot element is empty (no countdown when already at cap).
     *
     * This matters because there is no "next slot" once all 7 are unlocked.
    ...
    const { countEl, maxEl, nextEl } = makeElements();

    // WHEN
    await renderCapacityCounter(countEl, maxEl, nextEl, 0);

    // THEN
    expect(countEl.textContent).toBe(String(CAPACITY_CAP));
    expect(maxEl.textContent).toBe(String(CAPACITY_CAP));
...
describe('renderCapacityCounter — mid state', () => {
  it('shows current capacity and next-slot countdown when not at cap', async () => {
    /**
     ...
     * capacity and the next element shows the formatCapacityCountdown output.
     *
     * This matters because users need to know how many slots they have and
...
describe('renderCapacityCounter — petCount clamp reflects honest countdown', () => {
  it('shows petCount as capacity and next-slot countdown for slot petCount+1 when petCount > timeCapacity', async () => {
    /**
     ...
     * nextSlotMs must point to slot 5 (4 * CAPACITY_GROWTH_MS), not slot 2.
     *
     * If this breaks, the countdown shows the wrong time and re-enables Add too
     * early (slot 5 unlocks before it should), confusing users and breaking the gate.
     */
    ...
    // WHEN
    await renderCapacityCounter(countEl, maxEl, nextEl, 4);

    // THEN — capacity clamped to 4, nextSlotMs = 4 * CAPACITY_GROWTH_MS
    expect(countEl.textContent).toBe('4');
    expect(maxEl.textContent).toBe(String(CAPACITY_CAP));
    // next slot countdown should be non-empty (not at cap)
    expect(nextEl.textContent).not.toBe('');
    // should reflect ~12 days (4 * 3 days)
    expect(nextEl.textContent).toMatch(/next slot in \d+d \d+h/);
  });
  ...
  it('shows capacity 5 at day 12 when anchor was day 0 with petCount 4 (slot 5 just unlocked)', async () => {
     ...
     * and the user misses their newly unlocked housing slot.
     */
...
describe('renderCapacityCounter — null elements', () => {
  it('returns without error when DOM elements are null', async () => {
    ...
    // WHEN / THEN — no throw
    await expect(
      renderCapacityCounter(null, null, null, 0)
